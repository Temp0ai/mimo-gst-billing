        ...
        Column(modifier = Modifier.fillMaxSize().padding(padding).background(Background).verticalScroll(rememberScrollState()).navigationBarsPadding()) {
            Card(modifier = Modifier.fillMaxWidth().padding(16.dp), shape = RoundedCornerShape(16.dp), colors = CardDefaults.cardColors(containerColor = Color.White)) {
                Column(modifier = Modifier.fillMaxWidth().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    ...
                    Box { OutlinedTextField(value = printFormat, onValueChange = {}, readOnly = true, label = { Text("Print Format") }, trailingIcon = { Icon(Icons.Filled.ExpandMore, contentDescription = null, modifier = Modifier.clickable { showFormatDropdown = true }) }, modifier = Modifier.fillMaxWidth()); DropdownMenu(expanded = showFormatDropdown, onDismissRequest = { showFormatDropdown = false }) { listOf("A4", "A5", "Thermal 80mm", "Thermal 58mm").forEach { f -> DropdownMenuItem(text = { Text(f) }, onClick = { printFormat = f; save("format", f); showFormatDropdown = false }) } } }
                    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) { Text("Default Copies", fontSize = 14.sp, color = TextPrimary); Row { TextButton(onClick = { if (showCopies > 1) { showCopies--; save("copies", showCopies) } }) { Text("-", fontSize = 20.sp, color = Primary, fontWeight = FontWeight.Bold) }; Text(showCopies.toString(), fontSize = 16.sp, fontWeight = FontWeight.Bold, modifier = Modifier.padding(horizontal = 16.dp)); TextButton(onClick = { showCopies++; save("copies", showCopies) }) { Text("+", fontSize = 20.sp, color = Primary, fontWeight = FontWeight.Bold) } } }
                }
            }
